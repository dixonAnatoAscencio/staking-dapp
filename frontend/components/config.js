...
const masterchefAddr = '0x1Ba03c6E8c1d0290498728e915d3844829627b2E';
...
export const getPoolDetails = async () => {
    ...
    for (let i = 0; i < poolLength; i++){
        ...
        let userReward = Number(await convertToEth('reward', userRewardRaw)).toFixed('2');//cantidad de tokens que debemos pagarle al staker
...
export const action = async (i, amount, tokenaddress, action) => {
  try{
    ...
    if (action == "unstake") {
      ...
      let waitApproval = await approveTransfer.wait();//espera hasta hacer la aprobacion 
      if (waitApproval) {
